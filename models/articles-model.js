...
exports.selectArticle = async (article_id) => {
  const data = await db.query(
    "SELECT articles.author, title, articles.article_id, articles.body, topic, articles.created_at, articles.votes, article_img_url, COUNT(comment_id)::INT as comment_count FROM articles LEFT JOIN comments ON articles.article_id = comments.article_id WHERE articles.article_id = $1 GROUP BY articles.article_id",
    [article_id]
  );
...
exports.selectArticles = async (
  ...
  queryStrFiltered += format(" ORDER BY %I %s", sort_by, order);
  if (p) {
    if (limit) {
  ...
  if (total_count) {
    const dataFiltered = await db.query(queryStrFiltered, queryValuesFiltered);
    result.total_count = dataFiltered.rows.length;
  }
  return result;
};
...
exports.insertComment = async (article_id, username, body) => {
  ...
  const data = await db.query(
    ...
    [article_id, username, body, new Date().toISOString()]
  );
  return data.rows[0];
};
...
exports.insertArticle = async (author, title, body, topic, article_img_url) => {
  ...
  const data = await db.query(
    "INSERT INTO articles (author, title, body, topic, article_img_url, created_at, votes) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *;",
    [author, title, body, topic, article_img_url, new Date().toISOString(), 0]
  );
